...
export const onAuthStateChange = (callback: (user: User | null) => void): Subscription => {
    // If Supabase isn't configured, return a no-op subscription to prevent crashes.
    if (!supabase) {
        return {
            id: 'mock-subscription-no-client',
            unsubscribe: () => {},
            callback: () => {},
    ...
    // The subscription can be null if the listener fails.
    if (!subscription) {
        console.error("[AuthService] Failed to subscribe to auth state changes.");
        return {
            id: 'mock-subscription-failed',
            unsubscribe: () => {},
            callback: () => {},
        } as unknown as Subscription;
    }
...
export const getCurrentUser = async (): Promise<User | null> => {
    ...
    if (error) {
        console.error("[AuthService] Error fetching user session:", error.message);
        return null;
    }
    if (!session) {
        return null;
...
export const unlinkAccount = async (provider: 'google'): Promise<void> => {
    if (!supabase) {
        console.warn("[AuthService] Supabase client not initialized. Cannot unlink account.");
        return;
    }
